# Two Very Auto v3.0 - Terraform Infrastructure
# AWS EKS와 필수 리소스 프로비저닝
...
terraform {
  ...
  backend "s3" {
    bucket = "two-very-auto-terraform-state"
    key    = "production/terraform.tfstate"
    region = "ap-northeast-2"
    
    dynamodb_table = "two-very-auto-terraform-locks"
    encrypt        = true
  }
}
...
variable "domain_name" {
  ...
  type        = string
  default     = "two-very-auto.com"
}

# Local values
locals {
  tags = {
    Project     = "Two Very Auto"
...
resource "aws_elasticache_replication_group" "main" {
  ...
  description               = "Redis cluster for Two Very Auto"
...
resource "aws_s3_bucket" "backups" {
  bucket = "${var.cluster_name}-backups-${random_string.bucket_suffix.result}"
...
# IAM Role for EKS service account (used by apps)
resource "aws_iam_role" "eks_service_account" {
  ...
  assume_role_policy = jsonencode({
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub": "system:serviceaccount:two-very-auto-prod:two-very-auto-service-account"
        }
      }
    }]
    Version = "2012-10-17"
