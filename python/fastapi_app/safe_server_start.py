...
# 오류 방지 모듈 import
from error_prevention_wrapper import setup_drive_error_prevention, verify_system_integrity
...
def check_python_environment():
    ...
    try:
        ...
        for module in required_modules:
            try:
                __import__(module)
                print(f"✅ 모듈 {module} 사용 가능")
            except ImportError:
                print(f"❌ 모듈 {module} 없음")
                return False
        
        return True
        
    except Exception as e:
        print(f"❌ Python 환경 검증 실패: {e}")
        return False
...
def start_server_safely(port):
    ...
    try:
        ...
        print(f"   명령어: {' '.join(cmd[:2])} [uvicorn 명령어]")
        ...
        try:
            while True:
                ...
                try:
                    ...
                    print(f"⚠️ [{time.strftime('%H:%M:%S')}] 서버 응답 없음")
